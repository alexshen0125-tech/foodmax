# 供应商自助注册 产品需求文档（PRD）v1.0

**文档编号**：SUP-SELF-REG-001 | **版本**：v1.1 | **作者**：AI Product Manager | **创建日期**：2026-04-05 | **更新日期**：2026-04-05 | **状态**：Draft

> **v1.1 变更记录**：① 移除链接过期重发功能（F-14 原）；② 审核流程增加采购负责人 + 财务负责人二级审批；③ 新增表单字段灵活配置功能（F-14 新）。
...
将供应商注册信息采集流程线上化：采购在系统中生成唯一注册链接和二维码（自动绑定采购人信息），通过 WhatsApp / 微信发送给供应商；供应商通过手机端直接打开链接填写表单并上传资质（无需登录）；提交后采购员在中后台审核并可直接修改信息，完成后提交至采购负责人 → 财务负责人二级审批，全部通过后系统自动创建供应商档案。
...
### 场景 1：采购生成邀请链接（Generate Invite）— 优先级 P0

用户故事：「作为采购员，我希望在系统中输入供应商名称和联系人后，一键生成注册链接和二维码，链接自动携带我的采购人信息（姓名、站点、币种），以便我直接通过微信或 WhatsApp 发送给供应商。」

### 场景 2：供应商手机端填写（Supplier Mobile Form）— 优先级 P0

用户故事：「作为供应商联系人，我希望通过微信/WhatsApp 收到的链接直接在手机浏览器中打开表单（无需注册登录），分步完成企业信息、结算信息、资质上传，支持中途保存草稿，以便我利用碎片时间完成填写。」

### 场景 3：采购审核 + 二级审批建档（Review & Approval）— 优先级 P0

用户故事：「作为采购员，我希望在供应商提交信息后进入审核页面，对不合规内容直接编辑修改，补充内部管理字段后提交给采购负责人审批；采购负责人通过后再流转至财务负责人审核结算信息；全部审批通过后系统自动建档，全程有操作日志留痕。」

### 场景 4：表单字段配置（Field Configuration）— 优先级 P1

用户故事：「作为系统管理员，我希望在字段配置中心中随时新增、隐藏或调整供应商表单的字段及必填规则，配置保存后新生成的邀请链接即时生效，以便业务快速响应字段变化，无需等待研发排期。」

---

## 四、业务流程 / Business Flow
...
退回时审批人须填写退回原因（必填），系统通知采购员。采购员在审核页面修改对应字段后，可重新点击「提交审批」，审批从第一级（采购负责人）重新开始。
...
| 功能编号 | 功能名称 | 优先级 | 所属端 | 说明 |
|---------|---------|--------|--------|------|
| F-01 | 新建注册邀请 | P0 | 中后台 | 采购填写供应商名称、联系人、有效期，生成链接和二维码 |
| F-02 | 链接自动绑定采购信息 | P0 | 中后台 | 链接携带采购员、站点、币种，供应商端自动显示 |
...
| F-09 | 资质文件上传 | P0 | 前端（H5） | 支持拍照/相册上传，PDF/JPG/PNG，≤10MB |
...
### 5.2 供应商端字段清单 — 基本信息（默认配置，可通过 F-14 调整）

| 字段名 | 字段类型 | 默认必填 | 供应商可填 | 说明 |
|--------|---------|---------|-----------|------|
...
### 5.3 供应商端字段清单 — 结算信息（默认配置，可通过 F-14 调整）
...
## 六、交互说明 / Interaction Design

### 6.1 中后台 — 新建邀请

- 入口：供应商管理 > 注册邀请 > 「+ 新建邀请」按钮
- 弹窗表单字段：供应商名称（必填）、联系人（必填）、联系方式、链接有效期（7/14/30天）、备注
- 弹窗底部蓝色信息条：自动展示当前登录采购员的姓名、站点、币种
- 提交后弹出「链接与二维码」弹窗
...
| 操作 | 待审核→「去审核」/ 审批退回→「去修改」/ 待填写→「复制链接」「二维码」/ 已通过→「查看详情」/ 采购负责人或财务→「去审批」|

### 6.4 中后台 — 采购员审核页面

四个 Tab：

- **供应商信息**：展示供应商填写的全部字段（按配置中心字段版本渲染），采购员可直接编辑修改，修改内容系统记录留痕
- **内部补充字段**：供应商等级、合同扣点等仅采购可见的管理字段，黄色标签提示「仅采购填写 · 供应商不可见」
- **审批流程**：展示当前审批进度（采购员审核 → 采购负责人 → 财务负责人），每步状态（待审 / 通过 / 退回+原因）均可见
- **操作日志**：时间线展示全流程操作记录

页面底部固定操作栏：「提交审批」主按钮（首次提交及退回后重提均使用此按钮）

### 6.5 中后台 — 审批人页面（采购负责人 / 财务负责人）
...
## 七、功能详细设计 / Detailed Feature Design

### 7.1 F-02 链接自动绑定采购信息

链接结构：`https://scm.foodmax.com/s/{invite_token}`
...
供应商打开链接后，系统自动：
1. 验证链接有效性（未过期、未作废）
2. 按 field_config_version 加载对应字段配置渲染表单
3. 在表单顶部展示采购对接人信息
4. 预填供应商名称（只读）和联系人（可修改）
5. 默认选中对应币种

### 7.2 F-07 Open Graph 卡片预览
...
微信额外支持通过 wx.config + wx.updateAppMessageShareData 自定义分享卡片。
...
审批通知渠道：系统内消息 + 可选飞书/邮件通知。

### 7.5 F-13 审批通过自动建档

全部审批通过后系统自动执行：

1. 在供应商主表创建记录，生成供应商编号（旧）: `S{YYYYMMDD}00001` 和 供应商编号（新）: 自增序号
2. 供应商状态设为「合作」
3. 创建人 = 发起邀请的采购员，最终审批人 = 财务负责人
4. 同步至供应商列表，支持后续 查看 / 编辑 / 查看合同 / 可供仓库 操作
5. 触发通知（站内消息通知采购员建档完成；可选邮件/短信通知供应商）
...
## 九、异常与边界处理 / Exception & Edge Cases
...
| E-14 | 采购员审核时供应商同时在修改草稿 | 以供应商最新提交为准 | 供应商已更新信息，请刷新页面查看最新版本 |

---

## 十、RBAC 权限设计 / Permission Design

### 10.1 角色权限矩阵

| 功能模块 | 操作 | 采购员 | 采购负责人 | 财务负责人 | 系统管理员 |
|---------|------|--------|----------|----------|-----------|
| 注册邀请 | 新建邀请 | ✅ | ✅ | ❌ | ❌ |
